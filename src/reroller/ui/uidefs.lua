...
local Text = Troubadour.UI.create_text_node
local TroUIBox = Troubadour.UI.create_UIBox_generic_options_custom

function Troubadour.UIDEF.auto_reroll_menu_UI()
  local display_menu = UIBox({ definition = Troubadour.UIDEF.reroll_targets_list("Current Targets:"), config = {type = "cm"}})
  return
...
function Troubadour.UIDEF.reroll_targets_list(header)
  local display_list = Troubadour.UIDEF.get_display_list()
...
function Troubadour.UIDEF.get_display_list()
  ...
  for i = 1, #Troubadour.collection_targets do
    local target = G.P_CENTERS[Troubadour.collection_targets[i]].name or G.P_CENTERS[Troubadour.collection_targets[i]].original_key
...
function Troubadour.UIDEF.added_key_UI()
  local target_textnode = Text { ref_table = Troubadour.UI.targets, ref_value = 'added_target', scale = 0.3 }
...
function Troubadour.FUNCS.get_type_collection_UIBox_func(set)
  local func
...
function Troubadour.UI.config_from_collection()
  return create_UIBox_generic_options({
    ...
    contents = SMODS.Mods["Troubadour"].extra_tabs()[3].tab_definition_function().nodes})
end